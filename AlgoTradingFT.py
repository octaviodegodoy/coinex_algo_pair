...
def check_cointegration(symbolY,symbolX,periods):
    ...
    print(f" Assets correlation is {correlation} spreads is {z_scores_rolling[-1]}")
    return z_scores_rolling,coint_flag 
...
def plot_series(price_data,symbol_y,symbol_x):    
    ...
    z_scores_rolling,coint = check_cointegration(symbol_y,symbol_x,252)
    print(f"Is cointegrated ? {coint}")
    ...
    plt.subplot(2, 1, 2)
    plt.plot(price_data.index,z_scores_rolling, label='Rolling Z-scores', color='green', linewidth=1)
